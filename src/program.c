...
static void usage() {
    printf(
            "    Options are as follows:\n"
                    ...
                    "        -i <id>       Specify device id. You can list all devices with argument -l\n"
                    "        -h            Show this help\n");
}
...
static void parse_options(int argc, char **argv) {
    ...
    while ((c = getopt(argc, argv, "d:c:i:hl")) != -1) {
        switch (c) {
            case 'h':
                usage();
                exit(RESULT_OK);
    ...
    printf("ID:%i\n", DEVICE_ID);
    if (CONFIG_PATH == NULL || DEVICE_ID == NULL) {
        usage();
        exit(RESULT_INVALID_OPTIONS);
    }
}
...
int main(int argc, char **argv) {
    ...
    echo(VERBOSE_DEBUG, "DEVICE_ID: %c\n", DEVICE_ID);
